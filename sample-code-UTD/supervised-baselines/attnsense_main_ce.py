...
def parse_option():
    ...
    opt.model_path = './save/attnsense/{}_models'.format(opt.dataset)
    opt.tb_path = './save/attnsense/{}_tensorboard'.format(opt.dataset)
    ...
    opt.model_name = 'attnsense_{}_{}_lr_{}_decay_{}_bsz_{}_trial_{}'.\
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (input_data1, input_data2, labels) in enumerate(train_loader):
        ...
        output, weight1, weight2 = model(input_data1, input_data2)
...
def main():
    ...
    for trial_id in range(opt.trial):
        ...
        print("attnsense_result_labelrate_{:,}:".format(opt.label_rate))

        np.savetxt("attnsense_result_labelrate_{:,}_{}.txt".format(opt.label_rate, trial_id), confusion)
        np.savetxt("attnsense_record_acc_labelrate_{:,}_{}.txt".format(opt.label_rate, trial_id), record_acc)
