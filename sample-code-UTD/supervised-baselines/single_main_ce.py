...
from single_model import MyUTDmodel
...
def parse_option():
    ...
    parser.add_argument('--modality', type=str, default='inertial',
    ...
    opt.model_path = './save/{}/{}_models'.format(opt.modality, opt.dataset)
    opt.tb_path = './save/{}/{}_tensorboard'.format(opt.modality,opt.dataset)
    ...
    opt.model_name = 'single_{}_{}_lr_{}_decay_{}_bsz_{}_trial_{}'.\
...
def set_loader(opt):
    ...
    # train and test for single sensor
    if opt.modality == 'inertial':
        single_x_train = x_train_label_1
        single_x_test = x_test_1
    else:
        single_x_train = x_train_label_2
        single_x_test = x_test_2

    train_dataset = data.Unimodal_dataset(single_x_train, y_train)
    test_dataset = data.Unimodal_dataset(single_x_test, y_test)
...
def set_model(opt):
    ...
    model = MyUTDmodel(input_size=1, num_classes=opt.num_class, modality=opt.modality)
...
def main():
    ...
    for trial_id in range(opt.trial):
        ...
        print("{}_result_labelrate_{:,}_{}:".format(opt.modality, opt.label_rate, trial_id))

        print("single_result_{}_labelrate_{:,}_{}:".format(opt.modality, opt.label_rate, trial_id), confusion)
        np.savetxt("single_result_{}_labelrate_{:,}_{}.txt".format(opt.modality, opt.label_rate, trial_id), confusion)
